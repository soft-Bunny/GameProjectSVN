...
public class LevelControl : MonoBehaviour
{
    ...
    public class LevelData
    {
        ...
        public LevelData() // 생성자.
        {
            ...
            this.probability = new float[(int)Block.CARD.NORMAL_CARD_NUM];
            // 모든 종류의 출현확률을 우선 균등하게 해둔다.
            for (int i = 0; i < (int)Block.CARD.NORMAL_CARD_NUM; i++)
            {
                this.probability[i] =
                1.0f / (float)Block.CARD.NORMAL_CARD_NUM;
            }
        }
    ...
    public void loadLevelData(TextAsset level_data_text)
    {
        ...
        foreach (var line in lines)
        {
            ...
            foreach (var word in words)
            {
                ...
                {
                    case 0:
                        level_data.probability[(int)Block.CARD.HEART] =
                        float.Parse(word); break;
                    case 1:
                        level_data.probability[(int)Block.CARD.DIAMOND] =
                        float.Parse(word); break;
                    case 2:
                        level_data.probability[(int)Block.CARD.SPADE] =
                        float.Parse(word); break;
                    case 3:
                        level_data.probability[(int)Block.CARD.CLOBER] =
                        float.Parse(word); break;

                    //case 4:
                    //    level_data.probability[(int)Block.CARD.YELLOW] =
                    //    float.Parse(word); break;
                    //case 5:
                    //    level_data.probability[(int)Block.CARD.MAGENTA] =
                    //    float.Parse(word); break;
                    case 4:
