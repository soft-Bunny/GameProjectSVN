        ...
        { // 잡은 블록이 비었으면.
          // 나중에 주석 해제
            if (!this.is_has_falling_block())
            {
                ...
                {
                    // 마우스 버튼이 눌렸으면// blocks 배열의 모든 요소를 차례로 처리한다.
        ...
        else
        {
            ...
            foreach (BlockControl block in this.blocks)
            {
                ...
                if (this.checkConnection(block))
                {
                    ...
                    myAudio.PlayOneShot(changeSound); // 블럭 바꾸는 사운드
                }
            }
            if (ignite_count > 0) // 불붙은 개수가 0보다 크면.  // ＝한 군데라도 맞춰진 곳이 있으면.
            {
                ...
                //this.real_score_count.updateHeartScore();
                //this.real_score_count.updateDiamondScore();
                //this.real_score_count.updateSpadeScore();
                //this.real_score_count.updateCloberScore();
            ...
            for (int x = 0; x < Block.BLOCK_NUM_X; x++)
            {
                ...
                for (int y = 0; y < Block.BLOCK_NUM_Y - 1; y++)
                {
                    ...
                    for (int y1 = y + 1; y1 < Block.BLOCK_NUM_Y; y1++)
                    {
                        ...
                        // 블록을 교체한다.
                        myAudio.PlayOneShot(popSound); // 떨어질 때의 효과음
...
// 블록을 만들어 내고 가로 9칸, 세로 9칸에 배치한다.
    public void initialSetUp()
    {
        ...
        // 블록의 색 번호.
        int color_index = 0;
        Block.CARD color = Block.CARD.FIRST;
        ...
        { // 처음~마지막행
            ...
            { // 왼쪽~오른쪽
              // BlockPrefab의 인스턴스를 씬에 만든다.
                ...
                color_index =
                Random.Range(0, (int)Block.CARD.NORMAL_CARD_NUM);
            }
        }
    }
    ...
    {
        // 각각의 블록 색을 기억해 둔다.
        Block.CARD color0 = block0.color;
        Block.CARD color1 = block1.color;
        // 각각의 블록의 확대율을 기억해 둔다.
    ...
    public Block.CARD selectBlockColor()
    {
        Block.CARD color = Block.CARD.FIRST;
        // 이번 레벨의 레벨 데이터를 가져온다.
        LevelControl.LevelData level_data = this.level_control.getCurrentLevelData();
        ...
        color = (Block.CARD)i; // i번째 색을 반환한다.
        return (color);
    }
